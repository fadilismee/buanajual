...
/* ------------------------------------------------------------------ */
/* Berita acara transaksi buyback (src/data/beritaAcara.json)           */
/* ------------------------------------------------------------------ */

export const BeritaPhotoKindSchema = z.enum(["unit", "transfer", "doc"]);

export const BeritaItemSchema = z.object({
  name: z.string().min(1),
  grade: z.string().min(1),
  price: z.number().int().nonnegative(),
});

export const BeritaPhotoSchema = z.object({
  url: z.string().min(1),
  caption: z.string(),
  kind: BeritaPhotoKindSchema,
});

export const BeritaEntrySchema = z.object({
  id: z.string().min(1),
  ...
  items: z.array(BeritaItemSchema).min(1),
  total: z.number().int().nonnegative(),
  note: z.string(),
  photos: z.array(BeritaPhotoSchema),
});

export const BeritaAcaraDataSchema = z.object({
  entries: z.array(BeritaEntrySchema).min(1),
});
