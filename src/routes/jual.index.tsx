...
/* Lapisan stack foto hero lama dihapus — hero kini pakai HeroSlideshow
   (crossfade + Ken Burns, ganti otomatis). */
...
function HeroSlideshow() {
  // Gabung heroStack + gallery biar variasi foto banyak, unik, tanpa duplikat.
  ...
  return (
    <div
      ...
      role="region"
      aria-label="Slideshow lab dan barang masuk Buana Computer"
      aria-roledescription="carousel"
    >
      {/* Layer foto: crossfade + Ken Burns halus (slide aktif di-remount
          pakai key-nya index supaya animasi restart tiap ganti foto) */}
      {slides.map((s, i) => {
        ...
        return (
          <img
            ...
            src={s.src}
            alt={s.title ?? "Foto Buana Computer"}
      ...
      {/* Gradient bawah biar caption terbaca */}
      ...
      {/* Progress bar tipis di atas — indikator waktu slide berikutnya */}
      {!reducedMotion && (
        <div className="absolute inset-x-0 top-0 z-30 h-0.5 bg-black/10">
          <div
            key={index}
            className="h-full w-full origin-left bg-pri"
            style={{
              animation: paused
                ? "slide-progress 3.8s linear forwards paused"
                : "slide-progress 3.8s linear forwards",
            }}
          />
        </div>
      )}
  ...
  const stats = [
    {
      value: "1,840+",
      valueClass: "text-pri",
      label: "Komponen Rusak Di-salvage 2024",
    },
    {
      value: "15 Mnt",
      valueClass: "text-on-surface",
      label: "Rata-rata Uji Multi-tester",
    },
  ];
  return (
    <section className="relative w-full overflow-hidden bg-surface-lowest">
      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-2 gap-2.5 px-4 pb-10 sm:gap-4 sm:pb-14 lg:grid-cols-4">
        {stats.map((s) => (
          <div
            key={s.label}
            className="flex items-center gap-2.5 rounded-xl border border-outline-variant/50 bg-surface-low p-3 sm:p-4"
          >
            <span
    ...
    <section className="mx-auto w-full max-w-7xl px-4 py-10 sm:py-14">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        {grades.map((g) => (
          <div
            key={g.code}
            className="flex flex-col justify-between space-y-4 rounded-xl border border-outline-variant/40 bg-surface-lowest p-5 shadow-sm transition-all hover:border-pri/30 hover:shadow-md"
          >
  ...
  return (
    <div className="group flex flex-col justify-between gap-2 rounded-xl border border-outline-variant/40 bg-surface-lowest p-3 shadow-sm transition-all hover:border-pri/30 hover:shadow-md">
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <span
            className={`font-monotech rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${gradeBadgeClass(item.gradeTone)}`}
          >
            {item.grade}
          </span>
      ...
      <div className="space-y-10 pt-8">
        {flat.length === 0 && (
          <div className="rounded-xl border border-outline-variant/40 bg-surface-lowest p-10 text-center shadow-sm">
            ...
            <a
              href={`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(`Halo Buana Computer, saya mencari ${query.trim() || "hardware"} tapi tidak ada di price list`)}`}
              target="_blank"
              rel="noreferrer"
      ...
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((s) => (
            <div
              key={s.n}
              className="flex flex-col justify-between space-y-4 rounded-xl border border-outline-variant/40 bg-surface-low p-5 shadow-sm transition-all hover:border-pri/30 hover:shadow-md"
            >
              <div className="space-y-3">
...
function JualPage() {
  ...
  // Sinkronkan header/katalog dengan URL ?q= — back/forward tetap konsisten.
  useEffect(() => {
    setQuery(search.q ?? "");
  }, [search.q]);
