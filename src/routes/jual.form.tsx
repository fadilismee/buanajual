...
export const Route = createFileRoute("/jual/form")({
  ...
  ): { model?: string | undefined; category?: string | undefined } => ({
  ...
  return (
    <>
      <div className="space-y-3 rounded-xl bg-surface-high p-5">
        ...
        <div className="font-monotech flex items-center justify-between rounded-lg border border-outline-variant/40 bg-surface-lowest p-2 px-3 text-[13px] text-on-surface">
...
function StepHeader({
  ...
  return (
    <div
      id={`langkah-${n}`}
      className="flex scroll-mt-28 items-center justify-between border-b border-surface-container pb-3"
    >
...
function JualFormPage() {
  ...
  // Bersihkan object URL preview foto saat unmount / diganti.
  useEffect(() => {
      ...
      <section className="w-full bg-surface py-8 sm:py-10">
        <div className="mx-auto max-w-6xl px-4">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
            ...
            >
              {/* Lompat cepat antar langkah */}
              <nav
                aria-label="Navigasi langkah form"
                className="sticky top-[104px] z-30 -mx-1 flex gap-1.5 overflow-x-auto rounded-xl border border-outline-variant/40 bg-surface-lowest/95 p-2 shadow-sm backdrop-blur [scrollbar-width:none] lg:top-[112px] [&::-webkit-scrollbar]:hidden"
              >
                {stepLinks.map((s) => (
                  <a
                    key={s.n}
                    href={`#langkah-${s.n}`}
                    className="font-monotech flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-on-surface-variant transition-colors hover:bg-pri/10 hover:text-pri"
                  >
                    <span className="flex h-4 w-4 items-center justify-center rounded-full bg-surface-high text-[10px] text-on-surface">
                      {s.n}
                    </span>
                    {s.label}
                  </a>
              ...
              <div className="space-y-4 rounded-xl bg-surface-lowest p-6 shadow-sm">
                ...
                <label
                  htmlFor="photoUpload"
                  className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-outline-variant/60 bg-surface-low p-8 text-center transition-all hover:border-pri/40 hover:bg-pri/5"
                >
                ...
                {photos.length > 0 && (
                  <div className="space-y-2">
                    <ul className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                      {photos.map((p, i) => (
                        <li key={`${p.name}-${i}`} className="group relative">
                          <img
                            src={p.url}
                            alt={p.name}
                          ...
                          <button
                            ...
                            className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-tertiary text-xs font-bold text-on-tertiary opacity-0 shadow transition-opacity group-hover:opacity-100 focus-visible:opacity-100"
                          >
                            ×
                          </button>
                          <span className="font-monotech mt-1 block truncate text-[10px] text-outline">
                            {p.name}
                          </span>
                        </li>
                      ))}
              ...
              <div className="space-y-2">
                <button
                  type="submit"
                  className="font-heading sticky bottom-4 z-30 flex w-full items-center justify-center gap-2 rounded-xl bg-pri px-4 py-4 text-sm font-bold text-on-pri shadow-lg transition-all hover:bg-pri-container sm:text-base"
                >
                  <Send size={20} />
                ...
                {sent && (
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="mt-3 inline-flex rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-emerald-700"
                    >
                      Buka Chat WA Lagi
                    </a>
                  </div>
